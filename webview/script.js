...
document.querySelectorAll('.nav-btn').forEach(btn => {
    btn.addEventListener('click', () => navigateTo(/** @type {HTMLElement} */(btn).dataset.page));
});
...
document.getElementById('prs-container').addEventListener('click', e => {
    ...
    if (viewBtn) {
        selectPR(/** @type {HTMLElement} */(viewBtn).dataset.prKey,
                 /** @type {HTMLElement} */(viewBtn).dataset.prTitle);
    }
});

/* ── Issues table delegation (Fix, Resolve, Checkboxes) ─────────────────── */
...
function renderIssueRows(issues) {
    ...
    const rows = issues.map(/** @param {any} issue */ issue => {
        ...
        return `<tr id="irow-${esc(issue.key)}" class="${fixing ? 'row-fixing' : ''}${selected ? ' row-selected' : ''}">
            ...
            <td><span class="cell-key" title="${esc(issue.key)}">${esc(issue.key.substring(0, 14))}…</span></td>
            <td><span class="cell-file" title="${esc(filePath)}">${esc(filePath)}</span></td>
            <td><span class="cell-line">${line}</span></td>
            <td><span class="sev sev-${issue.severity}">${issue.severity}</span></td>
            <td><span class="cell-msg" title="${esc(issue.message)}">${esc(issue.message)}</span></td>
            ...
            <td>
                <div class="action-cell">
                    <button class="btn btn-primary btn-sm" id="fix-btn-${esc(issue.key)}"
                        ...
                        ${fixing ? '<span class="spinner" style="border-top-color:#fff"></span> Fixing…' : 'Fix'}
                    </button>
                    <button class="btn btn-secondary btn-sm" data-action="resolve"
                        data-issue-key="${esc(issue.key)}" title="Mark resolved in SonarQube">✓</button>
                </div>
            </td>
        </tr>`;
    }).join('');
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Key</th><th>File</th><th>Line</th>
                        <th>Severity</th><th>Message</th><th>Status</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
...
function exportIssues(format) {
    ...
    if (format === 'json') {
        content  = JSON.stringify(issues.map(i => ({
            key: i.key, file: i.component.includes(':') ? i.component.split(':').slice(1).join(':') : i.component,
        ...
        issues.forEach((/** @param {any} i */ i, idx) => {
            ...
            lines.push(
                `Issue ${idx + 1}:`,
                `  File:     ${f}`,
                `  Line:     ${i.textRange?.startLine ?? i.line ?? '?'}`,
                `  Severity: ${i.severity}`,
                `  Rule:     ${i.rule}`,
                `  Message:  ${i.message}\n`
            );
...
document.getElementById('btn-prev').addEventListener('click', () => {
    if (state.currentPage > 1) {
        state.currentPage--;
        fetchIssues(state.currentPRKey, state.currentPage);
    }
});
...
function fetchIssues(prKey, page) {
    vscode.postMessage({ type: 'fetchIssues', prKey, page });
}
...
function onLoadConfig(cfg) {
    ...
    if (cfg.fromFile) {
        document.getElementById('missing-props-warning')?.classList.add('hidden');
    }
}
...
function onLoading(key, active) {
    if (key === 'prs') {
        ...
        btn.querySelector('.btn-text').textContent = active ? 'Fetching…' : 'Fetch PRs';
        btn.disabled = active;
    }
    if (key === 'issues') {
        const container = document.getElementById('issues-container');
        ...
        if (active) {
            container.innerHTML =
                `<div class="loading-state"><div class="spinner"></div><span>Loading issues…</span></div>`;
    ...
    if (key === 'test') {
        ...
        btn.querySelector('.spinner').classList.toggle('hidden', !active);
        btn.querySelector('.btn-text').textContent = active ? 'Testing…' : 'Test Connection';
        btn.disabled = active;
    }
}
...
function renderPRs() {
    ...
    const sorted = [...prs].sort((a, b) => {
        ...
        if (col === 'branch'){ av = (a.branch || '').toLowerCase();   bv = (b.branch || '').toLowerCase(); }
        if (col === 'qg')    { av = a.status?.qualityGateStatus || ''; bv = b.status?.qualityGateStatus || ''; }
        if (col === 'date')  { av = a.analysisDate ? new Date(a.analysisDate).getTime() : 0;
    ...
    const rows = sorted.map(pr => {
        ...
        return `<tr class="pr-row" data-key="${esc(pr.key)}">
            <td><strong>${esc(pr.key)}</strong></td>
            <td>${esc(pr.title || pr.key)}</td>
            <td><code class="cell-file" style="max-width:160px;">${esc(pr.branch || '—')}</code></td>
            <td><span class="qg-${qg}">${qgIcon}${qg}</span></td>
            <td>${date}</td>
            <td>
                <button class="btn btn-primary btn-sm"
                    data-action="view-issues"
                    data-pr-key="${esc(String(pr.key))}"
                    data-pr-title="${esc(pr.title || String(pr.key))}">
                    View Issues
                </button>
            </td>
        </tr>`;
    ...
    container.innerHTML = `
        <div class="table-wrapper">
            <table>
                <thead>
                    <tr>
                        ${th('key','PR Key')}${th('title','Title')}${th('branch','Branch')}
                        ${th('qg','Quality Gate')}${th('date','Analysis Date')}<th>Action</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table>
...
function renderIssues(data) {
    ...
    if (state.currentPageIssues.length === 0) {
        const container = document.getElementById('issues-container');
        if (container) { container.innerHTML = '<div class="empty-state">No issues found for this PR.</div>'; }
...
function doFix(issueKey) {
    ...
    if (!issue) { return showToast('Issue data not found — refresh the page', 'error'); }
...
function setFixing(issueKey, active) {
    if (active) { state.fixingKeys.add(issueKey); }
    else        { state.fixingKeys.delete(issueKey); }
    ...
    if (btn) {
        ...
        btn.innerHTML = active
            ? '<span class="spinner" style="border-top-color:#fff"></span> Fixing…'
            : 'Fix';
    }
}
